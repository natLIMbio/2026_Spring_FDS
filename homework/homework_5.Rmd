...
date: Due Friday, February 20th, 2026
output: pdf_document
---

Now its time to practice what we have learned in class and learn even more! 
...
```{r}
suppressWarnings({library(dslabs)})
data(na_example)
mean(na_example)
sd(na_example)
```
...
25\. Use the `map_df` to create a data frame with three columns named `n`, `s_n`, and `s_n_2`. The first column should contain the numbers 1 through 100. The second and third columns should each contain the sum of 1 through $n$ and 1 through $n^2$ respectively, where $n$ the row number.  
